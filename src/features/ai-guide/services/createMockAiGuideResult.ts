...
export function createMockAiGuideResult({ coordinates }: CreateMockAiGuideResultParams): AiGuideResult {
  return {
    ...
    localGuideStory: 'Start from the camera scan screen to upload a real photo to the secure Supabase Edge Function and receive an AI Vision result or safe fallback.',
    interestingFacts: ['The production scan contract is validated before display.', 'OpenAI Vision runs only in the Supabase Edge Function, never in React Native code.'],
